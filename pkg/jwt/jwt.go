...
func GenerateRefreshToken(userID string) (string, error) {
	...
	err = config.RedisClient.Set(context.Background(), userID, refreshToken, expirationTime).Err()
	if err != nil {
		return "", err
	}
...
func ValidateRefreshToken(userID string, refreshToken string) (bool, error) {
	storedToken, err := config.RedisClient.Get(context.Background(), userID).Result()
	if err == redis.Nil {
		return false, nil
	} else if err != nil {
		return false, err
